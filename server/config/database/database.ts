...
export class Database {
  ...
  public static async connect(): Promise<void> {
    ...
    if (!HealthStatus.isDatabaseConnected) {
      try {
        // This is a flag that's used to remove a deprecation warning from mongo - https://github.com/Automattic/mongoose/issues/6890
        mongoose.set("useCreateIndex", true);
        await mongoose.connect(
          Config.active.get("mongoConnectionString"),
          connectionOptions
        );
        this.databaseName = mongoose.connection.db.databaseName;

        log.info(
          `Connected To Mongo Database: ${mongoose.connection.db.databaseName}`
        );

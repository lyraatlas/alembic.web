...
export class ConfigurationSchema {
  public static convictSchema: convict.Config = convict({
    ...
    isConsoleColored: {
      doc:
        "Colorization Affects the logging, so be careful on which environments you turn this on.",
      format: Boolean,
      default: true,
      env: "CONSOLE_COLOR"
    },
    mandrillApiKey: {
      doc:
        "DEFAULT HERE IS TEST KEY! This is the api key that we want to use with mandrill.  Keep in mind there is a different test for the integration environment",
      format: String,
      default: "",
    ...
    AlembicS3BucketRootUrl: {
      doc:
        "This is the root of the url that well use whenever a image is added.",
      format: String,
    ...
    clientDistFolder: {
      doc:
        "The client dist folder needs to be set, we build the docker image with all the client folders built.",
      format: String,
      default: "dist",
